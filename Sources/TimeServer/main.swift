...
private final class WebSocketTimeHandler: @unchecked Sendable, ChannelInboundHandler {
  ...
  private func sendTime(context: ChannelHandlerContext) {
    ...
    let foundationTime = Date()
    let buffer = context.channel.allocator.buffer(string: "Current Server Date: \(foundationTime.description)")
